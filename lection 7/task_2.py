...
class PersonInfo:
    """
    Класс сортирует данные о сотрудниках и подсчитывает индексацию зарплаты.
    """
    def __init__(self, name, age, *job):
        """
        Класс принимает данные о сотрудниках сортируя их по функциям
        :param name: Имя сотрудника.
        :param age: Возраст сотрудника
        :param job: Подразделения от головного до того, где работает сотрудник.
        """
        self.name = name
        self.age = age
        self.job = job
    ...
    def short_name(self):
        """
        Функция приводит имя, фамилию сотрудника к виду: Фамилия И.
        :return: Возвращаем отсортированную Фамилию И.
        """
        sh_name = self.name.split()
        sh_name = sh_name[1] + ' ' + sh_name[0][0] + '.'
        return sh_name

    def path_deps(self):
        """
        Добавляем в рабочем пути "-->" между папками
        :return: Возвращаем отсортированный путь работы сотрудника.
        """
        return ' --> '.join(self.job)

    def new_salary(self):
        """
        Индексируем зарплату сотрудника.
        :return: Возвращаем проиндексированную зп по формуле: 1337 * возраст * на сумму 3х самых часто
        встречаемых буквы в пути работы сотрудника.
        """
        self.job = ''.join(self.job)
        sort_sum = {item: self.job.count(item) for item in self.job}
        sort_sum = sorted(sort_sum.values())
        return 1337 * self.age * sum(sort_sum[-3:])
