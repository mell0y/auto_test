...
def repeats(our_str):
    """
    Функция изменят строку добавляя к букве число которое указывает.
     Какая это по счету повторяющаяся буква в строке, буква и число разделены "_"
    :param our_str: Принимаем на вход строку в которой будем считать количество повторений букв.
    :return: Возвращаем строку где у каждой буквы через "_" указано число соответствующее количеству
    уже встретившихся букв в строке.
    """
    letters = []
    for j in our_str:
        letters += j + '_'
        letters += str(letters.count(j))
